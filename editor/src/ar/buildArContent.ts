...
import { pbFileUrl } from '../lib/pb';
import type { ArContentRecord } from '../types';

const gltfLoader = new GLTFLoader();

/**
 * Build a Three.js Object3D for an ar_content record (editor preview version).
 * Mirrors apprenant's buildArContent so the visual editor renders WYSIWYG.
 */
export async function buildArContent(content: ArContentRecord): Promise<THREE.Object3D | null> {
  ...
  obj.userData.contentId = content.id;
  obj.traverse((o) => { o.userData.contentId = content.id; });
